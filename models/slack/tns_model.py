...
def main():
    ...
    parser.add_argument("-dfile", required=True, help="Provide the tns datafile")
    ...
    with open(args.dfile, 'r') as fh:
        ...
        for row in reader:
            ...
            if row['tns'] == 'NA':
                continue
            if float(row['clk_scaling']) != 0.8 or float(row['tns']) == 0.0:
                continue
            if row['ABC_MODE'] == 'NA':
                continue
        ...
        if "npy" in xnp.name:
            ...
            if xid in y_data:
                ...
                for row in y_data[xid]:
                    ...
                    if "NA" in param_vals or "NA" in row['tns']:
                        continue
                    ...
                    if args.eval in xid:
                        # y_val_list.append(float(row['tns'])*gauss(1, 0.02)/scale)
                        y_val_list.append(float(row['tns']))
                        X_val_list.append(x_data.tolist())
                    else:
                        # y_list.append(float(row['tns'])*gauss(1, 0.02)/scale)
                        y_list.append(float(row['tns']))
    ...
    # results.to_csv(f'tns_train_pred_7.csv')
    results.to_csv(f'tns_train_pred_45.csv')
    ...
    # with open('tns_model_nangate7.pkl','wb') as f:
    #     pickle.dump(regr,f)
    
    with open('tns_model_nangate45.pkl','wb') as f:
        pickle.dump(regr,f)
