...
def main():
    ...
    parser.add_argument("-dfile", required=True, help="Provide the area datafile")
    ...
    with open(args.dfile, 'r') as fh:
        ...
        for row in reader:
            ...
            if row['area'] == 'NA':
                continue
            # Only take rows wth "clk_scaling == 0.8, so we have much area"
            if float(row['clk_scaling']) != 0.8 or float(row['area']) == 0.0:
                continue
            if row['ABC_MODE'] == 'NA':
                continue
        ...
        if "npy" in xnp.name:
            ...
            if xid in y_data:
                ...
                for row in y_data[xid]:
                    ...
                    if "NA" in param_vals or "NA" in row['area']:
                        continue
                    ...
                    if args.eval in xid:
                        # y_val_list.append(float(row['area'])*gauss(1, 0.02)/scale)
                        y_val_list.append(float(row['area']))
                        X_val_list.append(x_data.tolist())
                    else:
                        # y_list.append(float(row['area'])*gauss(1, 0.02)/scale)
                        y_list.append(float(row['area']))
    ...
    regr = RandomForestRegressor(n_estimators=30, max_depth=12)
    ...
    results.to_csv(f'area_train_pred_45.csv')
    ...
    # with open('area_model_nangate7.pkl','wb') as f:
    #     pickle.dump(regr,f)

    with open('area_model_nangate45.pkl','wb') as f:
        pickle.dump(regr,f)
